...
const LibraryItemFormPage: React.FC = () => {
  ...
  const handleFieldChange = (name: string, value: any) => {
    setItem((prevItem) => {
      // Hantera booleska värden särskilt för checkboxar
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      if (item.id) {
        await updateLibraryItem(item.id, item); // Skicka både id och newItem
      } else {
      ...
      setError("Failed to save library item");
    }
  };
  ...
  const handleCheckOut = () => {
    ...
    if (
      ...
      const updatedItem: LibraryItemType = {
        ...
        borrowDate: new Date(), // Skapa en Date-instans
      };

      updateLibraryItem(item.id, updatedItem) // Skicka både id och updatedItem
        .then((response) => {
  ...
  const handleReturn = async () => {
    try {
      ...
      await updateLibraryItem(item.id, updatedItem); // Skicka både id och updatedItem
      setItem(updatedItem);
      setBorrowerName("");
    } catch (error) {
...
//   deleteLibraryItem,
//   getLibraryItem,
//   updateLibraryItem, // Denna funktion används för att uppdatera biblioteksposter
// } from "../services/libraryItemService";
...
//       updateLibraryItem(updatedItem) // Använd den importerade updateLibraryItem-funktionen
//         .then((response) => {
//           console.log("Item updated successfully", response);
//           setItem(updatedItem);
//           setBorrowerName("");
